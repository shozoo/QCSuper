...
class DeviceInfoCommand(BaseEfsShellCommand):
    ...
    def execute_command(self, diag_input, args: Namespace):
        ...
        if opcode != DIAG_SUBSYS_CMD_F:
            print(
                'Error executing DEV_INFO: %s received with payload "%s"'
        ...
        (
            ...
            device_id,
            device_type,
        ) = unpack(device_info_spec, payload[: calcsize(device_info_spec)])

        device_name: str = (
            payload[calcsize(device_info_spec) :].rstrip(b"\x00").decode("utf8")
        )

        if errno:
            print(
                "Error executing DEV_INFO: %s"

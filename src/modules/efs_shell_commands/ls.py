...
class LsCommand(BaseEfsShellCommand):
    ...
    def get_argument_parser(
        ...
        argument_parser = subparsers_object.add_parser(
            'ls', description='List files within a given directory of the EFS.'
        )
    ...
    def execute_command(self, diag_input, args: Namespace):
        ...
        if opcode != DIAG_SUBSYS_CMD_F:
            print(
                'Error executing OPENDIR: %s received with payload "%s"'
                % (message_id_to_name.get(opcode, opcode), repr(payload))
            )
            return

        (cmd_subsystem_id, subcommand_code, dir_fd, errno) = unpack(
            '<BHIi', payload
        )

        if errno:
            print(
                'Error executing OPENDIR: %s'
        ...
        try:  # Close the directory identifier in all cases using the "finally" block below
            ...
            while True:  # Iterate over directory files
                ...
                (
                    ...
                    mtime,
                    ctime,
                ) = unpack(readdir_struct, payload[: calcsize(readdir_struct)])

                entry_path = payload[calcsize(readdir_struct) :]

                if errno:
                    print(
                        'Error executing READDIR: %s'
                        % (EFS2_ERROR_CODES.get(errno) or strerror(errno))
                    )
                    return
                ...
                table_rows_to_print.append(
                    {
                        ...
                        'File name': repr(
                            entry_path.rstrip(b'\x00').decode('latin1')
                        )
                        + ((' -> ' + repr(real_path)) if real_path else ''),
                        'File size': str(size)
                        if entry_type != 0x01
                        else '',  # 0x01: "FS_DIAG_FTYPE_DIR - Directory file"
                        'Modification': datetime.fromtimestamp(mtime).strftime(
                            '%Y-%m-%d %H:%M:%S'
                        ),
                        # 'Access': datetime.fromtimestamp(atime).strftime('%Y-%m-%d %H:%M:%S'), # Was taking too much horizontal space
                        'Creation': datetime.fromtimestamp(ctime).strftime(
                            '%Y-%m-%d %H:%M:%S'
                        ),
                    }
                )
            ...
            if table_rows_to_print:
                ...
                for row in table_rows_to_print:
                    print(
                        '+ '
                        + ' | '.join(
                            row[column_name].ljust(
                                column_index_to_max_value_char_width[
                                    column_index
                                ],
                                ' ',
                            )

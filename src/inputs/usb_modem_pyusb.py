...
class UsbModemPyusbConnector(HdlcMixin, BaseInput):
    ...
    def __init__(self, dev_intf: PyusbDevInterface):
        ...
        try:
            status = self.dev_intf.device.is_kernel_driver_active(
                self.dev_intf.interface.index
            )
        except Exception:
            pass
        else:
        ...
        try:
            ...
            self.dev_intf.device.set_configuration(
                self.dev_intf.configuration.bConfigurationValue
            )
        except USBError:
            pass
